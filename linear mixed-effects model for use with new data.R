...
plotMagFig<-function(dTablePred,dTable,degree,colormap,dTave) {
  ...
  xlimMag=c(min(dTable$freq),max(dTable$freq))
  nLevel=length(unique(dTable$level))
  cohorts=as.character(unique(dTable$cohort))
  if (nLevel==1) {
    fit<-lmer(magLog ~ stats::poly(freq,degree) + cohort + (1|id), data=dTable)
    eqn=sprintf("log(mag) ~ poly(freq,%d) + cohort + (1/id)",degree)
  } else {
    fit<-lmer(magLog ~ stats::poly(freq,degree)*level + cohort + (1|id), data=dTable)
...
# Analyze and plot phase data
plotPhFig<-function(dTablePred,dTable,degree,colormap,dTave) {
  ylimPh=c(-5,0)
  xlimMag=c(min(dTable$freq),max(dTable$freq))
  nLevel=length(unique(dTable$level))
  cohorts=as.character(unique(dTable$cohort))
  fit<-lmer(phaseCycle ~ stats::poly(freq,degree)*level + cohort + (1|id), data=dTable)
...
predDataTable<-function(dTable) {
  freqs=seq(min(dTable$freq),max(dTable$freq),length.out=100)
  levels=sort(unique(dTable$level))
  cohorts=sort(unique(dTable$cohort))
  nFreq=length(freqs)
  nLevel=length(levels)
...
dTablePred<-predDataTable(dTable)
...
print(dSet1$pSum) # magnitude coefficients and p-values
print(dSet2$pSum) # phase coefficients and p-values
